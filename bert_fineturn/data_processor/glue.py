...
class SickRegProcessor(SickProcessor):
    """Processor for the STS-B data set (GLUE version)."""
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "SICK_train.txt")), "train")
...
class WikiQAProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "WikiQA-train.tsv")), "train")
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "WikiQA-dev.tsv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "WikiQA-test.tsv")), "test")
...
class DBPediaProcessor(DataProcessor):
    ...
    def _create_examples(self, filename, set_type):
        ...
        for k, row in df.iterrows():
            ...
            guid = "%s-%s" % (set_type, str(k))
            examples.append(InputExample(guid=guid, text_a=row["title"], text_b=row["review"], label=label))
...
class TRECProcessor(DataProcessor):
    ...
    def _create_examples(self, filename, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        # InputExample(guid=guid, text_a=row["title"], text_b=row["review"], label=label)
        with open(filename, "rb") as f:
            for id_, row in enumerate(f):
                ...
                sid = set_type + '-' + str(id_)
                examples.append(InputExample(guid=sid, text_a=text, text_b=None, label=coarse_label))
        return examples
...
def glue_compute_metrics(task_name, preds, labels):
    ...
    elif task_name in ["sst-2", "mnli", "mnli-mm", "qnli", "rte", "wikiqa", 'trec']:
